...
> Explainer: "issue tracker" 是这个 repo 存放 issues 的地方。`to-tickets` 和 `triage` 等 skills 会从中读取并写入；它们需要知道是调用 `gh issue create`、在 `.scratch/` 下写 markdown 文件，还是遵循你描述的其他工作流。请选择你实际用于跟踪这个 repo 工作的位置。
