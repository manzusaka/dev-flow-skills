...
export class ShowCommand {
  ...
  private async showDirect(
    ...
    if (!params.typeOverride && isChange && isSpec) {
      ...
      if (isStoreSelectedRoot(root)) {
        ...
        console.error('传递 --type change|spec，或使用：spect change show / spect spec show');
      }
      process.exitCode = 1;
      return;
    }
  ...
  private printNonInteractiveHint(root: ResolvedOpenSpecRoot): void {
    ...
    console.error(`  ${withStoreFlag(root, 'spect show <item>')}`);
    if (isStoreSelectedRoot(root)) {
      // The noun-form commands are cwd-based and cannot reach a selected store.
      console.error(`  ${withStoreFlag(root, 'spect show <item> --type change')}`);
      console.error(`  ${withStoreFlag(root, 'spect show <item> --type spec')}`);
    } else {
      console.error('  spect change show');
      console.error('  spect spec show');
    }
