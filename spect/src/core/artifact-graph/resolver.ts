...
import { DEFAULT_OPENSPEC_SCHEMA } from '../openspec-root.js';
...
export function getPackageSchemasDir(): string {
  // Single-file bundles have no package directory on disk; schema content
  // reaches projects through `spect init` instead. Return a path that never
  // exists so callers fall through to project/user lookups.
  if (typeof import.meta.url !== 'string') {
    return path.join(path.sep, '.spect-bundle', 'schemas');
  }
...
export function getSchemaDir(
  ...
  if (projectRoot) {
    ...
    // dev-skills flat layout (written by `spect init`): the default schema
    // lives directly at openspec/schemas/schema.yaml with templates/ beside
    // it, without a per-schema subdirectory.
    if (name === DEFAULT_OPENSPEC_SCHEMA) {
      const flatDir = getProjectSchemasDir(projectRoot);
      if (fs.existsSync(path.join(flatDir, 'schema.yaml'))) {
        return flatDir;
      }
    }
  }
...
export function listSchemas(projectRoot?: string): string[] {
  ...
  if (projectRoot) {
    ...
    if (fs.existsSync(projectDir)) {
      // dev-skills flat layout: a schema.yaml directly under the schemas dir
      // serves the default schema.
      if (fs.existsSync(path.join(projectDir, 'schema.yaml'))) {
        schemas.add(DEFAULT_OPENSPEC_SCHEMA);
      }
...
export function listSchemasWithInfo(projectRoot?: string): SchemaInfo[] {
  ...
  if (projectRoot) {
    const projectDir = getProjectSchemasDir(projectRoot);
    if (fs.existsSync(projectDir)) {
      // dev-skills flat layout: schema.yaml directly under the schemas dir.
      const flatSchemaPath = path.join(projectDir, 'schema.yaml');
      if (fs.existsSync(flatSchemaPath)) {
        try {
          const schema = parseSchema(fs.readFileSync(flatSchemaPath, 'utf-8'));
          schemas.push({
            name: DEFAULT_OPENSPEC_SCHEMA,
            description: schema.description || '',
            artifacts: schema.artifacts.map((a) => a.id),
            source: 'project',
          });
          seenNames.add(DEFAULT_OPENSPEC_SCHEMA);
        } catch {
