...
function rerunCommand(
  ...
  if (changeName.startsWith('-')) {
    return `${withStoreFlag(root, `spect archive ${flags}`)} -- ${quoteChangeName(changeName)}`;
  }
  return withStoreFlag(root, `spect archive ${quoteChangeName(changeName)} ${flags}`);
}
    ...
    if (!changeName) {
      if (json) {
        throw new ArchiveBlockedError(
          ...
          withStoreFlag(root, 'spect archive <change-name> --json')
        );
      }
    ...
    if (!skipValidation) {
      ...
      if (hasValidationErrors) {
        if (json) {
          throw new ArchiveBlockedError(
            ...
            `变更 '${changeName}' 验证失败。`,
            `运行 ${withStoreFlag(root, `spect validate ${changeName}`)} 查看详情，修复错误，或使用 --no-validate 重新运行。`
          );
        }
        console.log(chalk.red('\n验证失败。归档前请修复错误。'));
      ...
      if (!options.yes) {
        throw new ArchiveBlockedError(
          ...
          withStoreFlag(root, 'spect archive <change-name> --json --no-validate --yes')
        );
      }
    } else {
        ...
        if (!options.yes) {
          if (json) {
            throw new ArchiveBlockedError(
              ...
              withStoreFlag(root, 'spect archive <change-name> --json --yes')
            );
          }
              ...
              if (!report.valid) {
                ...
                if (json) {
                  throw new ArchiveBlockedError(
                    ...
                    refusalReason ??
                      retirementHint ??
                      `修复 change deltas 后运行 ${withStoreFlag(root, `spect validate ${specName}`)}。`
                  );
                }
  ...
  private async selectChange(
    ...
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      throw new ArchiveBlockedError(
        ...
        withStoreFlag(root, `spect archive <change-name> ${rerunFlags(options).join(' ')}`)
      );
    }
